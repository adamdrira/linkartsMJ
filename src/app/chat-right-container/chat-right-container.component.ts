...
export class ChatRightContainerComponent implements OnInit {
  ...
  ngOnChanges(changes: SimpleChanges) {
    if(this.change_number>0){
      if(this.current_friend_id!=this.friend_id || this.friend_type!= this.current_friend_type){
        ...
        this.chatService.get_size_of_files(this.friend_id,this.id_chat_section,this.friend_type).pipe( first() ).subscribe(l=>{
          this.total_size_of_files[0]=Number(l[0][0].total);
         
        })

        this.chatService.get_size_of_pictures(this.friend_id,this.id_chat_section,this.friend_type).pipe( first() ).subscribe(r=>{
  ...
  ngOnInit(): void {
    ...
    this.ChatComponent.reload_list_of_archives.pipe(takeUntil(this.ngUnsubscribe)).subscribe(event=>{
      if(event.reload){
        ...
        this.chatService.get_chat_folders(this.chat_friend_id).pipe( first() ).subscribe(r=>{
          if(r[0] && r[0].length>0){
        ...
        let index_pic = this.list_of_pictures_data.findIndex(x => x.attachment_name === event.message.attachment_name && x.attachment_type === event.message.attachment_type);
        let index_file = this.list_of_files_data.findIndex(x => x.attachment_name === event.message.attachment_name && x.attachment_type === event.message.attachment_type);
        
        if(index_pic>=0){
          if(event.remove){
            this.list_of_pictures_data[index_pic].id_folder=0;
          }
          else{
            this.list_of_pictures_data[index_pic].id_folder=event.message.id_folder;
          }
        }
        if(index_file>=0){
          if(event.remove){
            this.list_of_files_data[index_file].id_folder=0;
          }
          else{
            this.list_of_files_data[index_file].id_folder=event.message.id_folder;
          }
        }
      }
    })
      ...
      if(this.myScrollContainer.nativeElement.scrollTop>=(this.myScrollContainer.nativeElement.scrollHeight-this.myScrollContainer.nativeElement.getBoundingClientRect().height)*0.8 -100 ){
            ...
            if(r[0][0]){
              ...
              let compt=0;
                for(let i=0;i<r[0].length;i++){
                  this.chatService.get_attachment_popup(r[0][i].attachment_name,(this.friend_type=='user')?'user':'group',(r[0][i].chat_friend_id)?r[0][i].chat_friend_id:this.chat_friend_id).pipe( first() ).subscribe(t=>{
                    let url = (window.URL) ? window.URL.createObjectURL(t) : (window as any).webkitURL.createObjectURL(t);
                    const SafeURL = this.sanitizer.bypassSecurityTrustUrl(url);
                    this.list_of_files_src[length+i]=SafeURL;
                    this.list_of_files_data[length+i]=r[0][i];
                    compt++;
                    if(compt==r[0].length){
            ...
            if(r[0][0]){
              ...
              for(let i=0;i<r[0].length;i++){
                ...
                if(re.exec(r[0][i].attachment_name)[1].toLowerCase()=="svg"){
                  ...
                  this.chatService.get_attachment_right(r[0][i].attachment_name,(this.friend_type=='user')?'user':'group',(r[0][i].chat_friend_id)?r[0][i].chat_friend_id:this.chat_friend_id).pipe( first() ).subscribe(t=>{
                    let url = (window.URL) ? window.URL.createObjectURL(t) : (window as any).webkitURL.createObjectURL(t);
                    const SafeURL = this.sanitizer.bypassSecurityTrustUrl(url);
                    this.list_of_pictures_src[length+i]=SafeURL;
                    this.list_of_pictures_data[length+i]=r[0][i];
        ...
        if(this.panelOpenState_2 && !this.show_scroll_archives && this.show_files_of_folder.findIndex(x => x)>=0){
          ...
          if(this.can_get_other_files_from_archive[index]){
            ...
            var re = /(?:\.([^.]+))?$/;
            this.chatService.get_files_by_folder(this.list_of_folders[index].id,this.offset_by_folder[index]).pipe( first() ).subscribe(m=>{
              let r=[]
              if(m[0].messages && m[0].messages.length>0){
                ...
                for(let i=0;i<r[0].length;i++){
                  ...
                  if((re.exec(r[0][i].attachment_name)[1].toLowerCase()=="svg" && r[0][i].attachment_type=="picture_attachment") || r[0][i].attachment_type=="file_attachment" ){
                    ...
                    this.chatService.get_picture_sent_by_msg(r[0][i].attachment_name).pipe( first() ).subscribe(t=>{
                      let url = (window.URL) ? window.URL.createObjectURL(t) : (window as any).webkitURL.createObjectURL(t);
                      const SafeURL = this.sanitizer.bypassSecurityTrustUrl(url);
                      this.list_of_files_by_folder[index][i+length]=SafeURL;
                      this.list_of_files_types_by_folder[index][i+length]="picture_attachment";
  ...
  get_files_and_pictures(id_chat_section){
    this.chatService.get_all_files("now",this.friend_id,id_chat_section,this.friend_type).pipe( first() ).subscribe(l=>{
      this.list_of_files=l[0];
      let compt=0;
      if(l[0].length>0){
        for(let i=0;i<l[0].length;i++){
          this.chatService.get_attachment_popup(l[0][i].attachment_name,(this.friend_type=='user')?'user':'group',(l[0][i].chat_friend_id)?l[0][i].chat_friend_id:this.chat_friend_id).pipe( first() ).subscribe(r=>{
            let url = (window.URL) ? window.URL.createObjectURL(r) : (window as any).webkitURL.createObjectURL(r);
            const SafeURL = this.sanitizer.bypassSecurityTrustUrl(url);
            this.list_of_files_src[i]=SafeURL;
            this.list_of_files_data[i]=l[0][i]
            compt++;
            if(compt==l[0].length){
    ...
    this.chatService.get_all_pictures("now",this.friend_id,id_chat_section,this.friend_type).pipe( first() ).subscribe(l=>{
  ...
  show_images(indice){
    ...
    if(indice<=7){
      for(let i =0;i<15;i++){
        if(this.list_of_pictures_data[i]){
          list_of_pics.push(this.list_of_pictures_names[i]);
          list_of_chat_friend_ids.push((this.list_of_pictures_data[i].chat_friend_id)?this.list_of_pictures_data[i].chat_friend_id:this.chat_friend_id)
        }
      }
    }
    else{
      indice=7;
      for(let i =indice-7;i<indice+7;i++){
        if(this.list_of_pictures_data[i]){
          list_of_pics.push(this.list_of_pictures_names[i]);
          list_of_chat_friend_ids.push((this.list_of_pictures_data[i].chat_friend_id)?this.list_of_pictures_data[i].chat_friend_id:this.chat_friend_id)
        }
  ...
  @Output() archive_from_right_emit = new EventEmitter<any>();
  ...
  get_files_by_folder(index){
    if(this.list_of_files_retrieved_by_folder[index]){
      ...
      this.chatService.get_files_by_folder(this.list_of_folders[index].id,0).pipe( first() ).subscribe(m=>{
        let r=[]
        if(m[0].messages && m[0].messages.length>0){
          r[0]=m[0].messages;
  ...
  unarchive(i,j){
    ...
    this.chatService.unarchive_chat_message(id_message,this.list_of_folders[i].id).pipe( first() ).subscribe(r=>{
  ...
  remove_folder(index){
    if(this.list_of_folders[index].id_user!=this.current_user_id){
      ...
      dialogRef.afterClosed().pipe( first() ).subscribe(result => {
        if(result){
          this.list_of_folders_retrieved=false;
          this.chatService.remove_folder(this.list_of_folders[index].id,this.chat_friend_id).pipe( first() ).subscribe(r=>{
            this.list_of_folders_retrieved=true;
              this.list_of_folders_opened=[];
              this.show_files_of_folder=[];
              this.list_of_files_retrieved_by_folder=[];
  ...
  manage_archive(index,archive,type){
    if(type=="picture"){
      if(archive){
        ...
        dialogRef.afterClosed().pipe( first() ).subscribe(result => {
          if(result){
            ...
            this.reset_folders()
            this.archive_from_right_emit.emit({id_message:this.list_of_pictures_data[index].id,id_folder:this.list_of_pictures_data[index].id_folder});
          }
        });
      }
      else{
        this.chatService.unarchive_chat_message(this.list_of_pictures_data[index].id,this.list_of_pictures_data[index].id_folder).pipe( first() ).subscribe(r=>{
      ...
      if(archive){
        ...
        this.chatService.unarchive_chat_message(this.list_of_files_data[index].id,this.list_of_files_data[index].id_folder).pipe( first() ).subscribe(r=>{
          this.list_of_pictures_data[index].id_folder=0;
