      ...
      if(!valid) {
        ...
        for (let step = 0; step < this.componentRef.length; step++) {
          ...
          this.componentRef[ step ].instance.sendImageUploaded.pipe(takeUntil(this.ngUnsubscribe)).subscribe( v => {
            this.navbar.add_page_visited_to_history(`/onComplete_swiper_bd_serie`,(v.file._file.size/1024/1024).toString() + " et " + v.sizeResponse.toString()).pipe( first() ).subscribe();
